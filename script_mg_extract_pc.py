...
dat=["TanBeta","gBL","gYB","mC12","mC22","x1","x2","hh_2","pphhaabb_xsection","ppzz4l_xsection"];
par=["TanBeta","gBL","gYB","mC12","mC22","x1","x2"];
mass=["mh2"];
xsec=["pphhaabb_xsection","ppzz4l_xsection"];

#Copy scan results to DB
for i in range(0,n):
	if(os.path.exists(ssp+"BLSSMScanLowmh"+str(i))):
#		shutil.move(ssp+"BLSSMScanLowmh"+str(i),"/home/mashry/Dropbox/00/BLSSMScanLowmh"+str(i))
		copy_tree(ssp+"BLSSMScanLowmh"+str(i),"/home/mashry/Dropbox/00/BLSSMScanLowmh"+str(i))
		print("DB/BLSSMScanLowmh"+str(i))

#Calling the shell script for splitting
subprocess.call(['sh','./script_1_split.sh'])
...
for i in range(0,n):
	for j in range(0,m):
		if(os.path.isfile(mg+"pphhaabb_blssm_low/pphhaabb_blssm_"+str(i)+"_"+str(j)+"/Events/run_01/run_01_tag_1_banner.txt")):
			shutil.move(mg+"pphhaabb_blssm_low/pphhaabb_blssm_"+str(i)+"_"+str(j)+"/Events/run_01/run_01_tag_1_banner.txt","/home/mashry/Dropbox/00/pphhaabb/"+"run_"+str(i)+"_"+str(j)+".txt")
	...
	for j in range(0,m):
		if(os.path.isfile(mg+"pphhaabb_blssm_low/run_"+str(i)+"_"+str(j)+".txt")):
			...
			for k in range(len(dat)-2):
				open(ssp+dat[k]+".dat", "a").writelines([l.replace("  # "+dat[k],"") for l in open(mg+"pphhaabb_blssm_low/run_"+str(i)+"_"+str(j)+".txt").readlines() if "# "+dat[k]+"\n" in l]) 
			print("pphhaabb_"+str(i)+"_"+str(j))
...
for i in range(0,n):
	for j in range(0,m):
		if(os.path.isfile(mg+"ppzz4l_blssm_low/ppzz4l_blssm_"+str(i)+"_"+str(j)+"/Events/run_01/run_01_tag_1_banner.txt")):
			shutil.move(mg+"ppzz4l_blssm_low/ppzz4l_blssm_"+str(i)+"_"+str(j)+"/Events/run_01/run_01_tag_1_banner.txt","/home/mashry/Dropbox/00/ppzz4l/"+"run_"+str(i)+"_"+str(j)+".txt")
...
for i in range(0,n):
	for j in range(0,m):
		if(os.path.isfile(mg+"ppzz4l_blssm_low/run_"+str(i)+"_"+str(j)+".txt")):
			...
			for k in range(len(dat)-2):
				open(ssp+dat[k]+".dat", "a").writelines([l.replace("  # "+dat[k],"") for l in open(mg+"ppzz4l_blssm_low/run_"+str(i)+"_"+str(j)+".txt").readlines() if "# "+dat[k]+"\n" in l]) 
			print("ppzz4l_"+str(i)+"_"+str(j))
...
for i in range(0,n):
    for j in range(0,m):
        if(os.path.isfile(ssp+"BLSSMScanLowmh/SPheno.spc.BLSSM_"+str(i)+"_"+str(j))):
            myfile.write("generate p p > h2 > h1 h1, h1 > a a, h1 > d3 d3bar\n")
...
for i in range(0,n):
    for j in range(0,m):
        if(os.path.isfile(ssp+"BLSSMScanLowmh/SPheno.spc.BLSSM_"+str(i)+"_"+str(j))):
            myfile.write("generate p p > h2, (h2 > z z, (z > l+ l-, z > l+ l-))\n")
